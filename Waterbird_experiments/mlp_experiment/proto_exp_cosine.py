...
def get_dist(embs, prototypes):
    dists = []
    for p in prototypes:
        cos_sim = np.dot(embs, p)/(np.linalg.norm(embs, axis=-1)*np.linalg.norm(p))
        dists.append(cos_sim)
        
    final_dist = np.max(dists, axis=0)
    return final_dist
...
ind_dists = 1 - get_dist(test_embs, train_prototypes)
ood_dists = 1 - get_dist(ood_embs, train_prototypes)
dist_utils.calc_ROC_with_dists(ind_dists, ood_dists)
...
ind_dists = 1 - get_dist(test_embs, aug_prototypes)
ood_dists = 1 - get_dist(ood_embs, aug_prototypes)
print('after:')
dist_utils.calc_ROC_with_dists(ind_dists, ood_dists)
