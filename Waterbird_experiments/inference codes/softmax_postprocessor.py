...
class PrototypicalPostprocessor(BasePostprocessor):
    def __init__(self, config, perform_normalization=True,
                 perform_val=False, perform_nn=False):
        ...
        if perform_val:
            self.retrain_split = 'val'
        else:
            self.retrain_split = 'train'
            
            
        if perform_nn:
            self.clf = MLPClassifier(hidden_layer_sizes=(32,))
        else:
    ...
    def setup(self, net: nn.Module, id_loader_dict, ood_loader_dict):
        ...
        net.eval()
        with torch.no_grad():
            for batch in tqdm(id_loader_dict[self.retrain_split],
                              desc='Setup: ',
                              position=0,
                              leave=True):
                feat_ = batch[0]
        ...
        self.clf.fit(self.train_feats, self.train_labels)
    ...
    def inference(self,
        ...
        pred_list = np.argmax(probs, axis=1).astype(int)
        conf_list = np.max(probs, axis=1)
