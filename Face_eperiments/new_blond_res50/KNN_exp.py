...
in_data_embs0 = np.load(f'new_embs/celeba_embs_resnet_50_0.9_seed10_new.npy', allow_pickle=True).item()
    
ood_embs0 = np.load(f'new_embs/clb_ood_new.npy', allow_pickle=True).item()
...
grouped_embs_train0 = grouped_embs_train0.copy()
...
print(sorted(list(train_dict.keys())))
print([len(train_dict[key]) for key in sorted(list(train_dict.keys()))])
print([len(test_dict[key]) for key in test_dict.keys()])
print([len(ood_dict[key]) for key in ood_dict.keys()])
