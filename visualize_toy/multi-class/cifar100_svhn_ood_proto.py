...
data = np.load("cifar100_resnet50_embeddings.npy")  # shape: (100, 500, 2048)
svhn = np.load("svhn.npy")  # shape: (1000, 2048)

data = data / np.linalg.norm(data, 2, axis=-1, keepdims=True)
svhn = svhn / np.linalg.norm(svhn, 2, axis=-1, keepdims=True)
...
for K in Ks:
    ...
    for seed in seeds:
        ...
        for cls in test_classes:
            X_test_id.append(data[cls][200:250])
        X_test_id = np.vstack(X_test_id)

        # Use fixed SVHN features as OOD samples
        X_ood = svhn[:500]  # Use first 500 SVHN embeddings
