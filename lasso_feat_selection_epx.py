...
for e in tqdm.tqdm(range(500)):
    ...
    if e % 100 == 0:
        with torch.no_grad():
            ...
            data0 = torch.tensor(np.concatenate([grouped_embs['woman_black'][:1000],
                                                 grouped_embs['woman_blond'][:1000]]),
                                                dtype=torch.float32).to(device)
            lbl0 = torch.zeros(2000, dtype=torch.long).to(device) 
...
merged_negative_feats = list(set(np.concatenate([woman_feats, man_feats])))

print(len(merged_feats), len(merged_negative_feats))

final_feats = [feat for feat in merged_feats if feat not in merged_negative_feats]
...
x_train = np.concatenate([data0, data1])[:, final_feats]
...
x_eval = np.concatenate([data0, data1])[:, final_feats]
