...
train_embs = np.concatenate([grouped_embs_train0[key] for key in grouped_embs_train0.keys()])
norm = np.linalg.norm(train_embs, axis=0, keepdims=True)

if normalize_embs:
    train_dict = {key: normalize(grouped_embs_train0[key], norm) for key in grouped_embs_train0.keys()}
    test_dict = {key: normalize(grouped_embs0[key], norm) for key in grouped_embs0.keys()}
    ood_dict = {key: normalize(ood_embs0[key], norm) for key in ood_embs0.keys()}
else:
