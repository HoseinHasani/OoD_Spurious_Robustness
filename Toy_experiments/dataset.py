...
#        core_axis = self.normalize(2 * np.random.rand(2) - 1, True)
#        sp_axis = self.normalize(2 * np.random.rand(2) - 1, True)
        core_axis = np.array([1, 0])
        sp_axis = np.array([0, 1])
        return core_axis, sp_axis
    
    def generate_dataset(self, alpha=0.6):
        
        mean = -alpha * self.sp_ax + 1 * self.core_ax
        min0 = np.random.normal(mean, self.std_min, size=(self.min_size, 2))
        min0 = self.normalize(min0)
        
        mean = (1 - alpha) * self.sp_ax + 1 * self.core_ax
        maj0 = np.random.normal(mean, self.std_maj, size=(self.maj_size, 2))
        maj0 = self.normalize(maj0)

        mean = alpha * self.sp_ax - 1 * self.core_ax
        min1 = np.random.normal(mean, self.std_min, size=(self.min_size, 2))
        min1 = self.normalize(min1)
        
        mean = -(1 - alpha) * self.sp_ax - 1 * self.core_ax
        maj1 = np.random.normal(mean, self.std_maj, size=(self.maj_size, 2))
...
class GaussianDataset3D():
    ...
    def generate_dataset(self, alpha=0.6):
        ...
        cr_coefs = np.dot(ood0, self.core_ax)
        ood0 -= 0.7 * cr_coefs[:, None] * np.repeat(self.core_ax[None], ood0.shape[0], axis=0)
        ood0 = self.normalize(ood0)
